...
$cmd = $ssh.RunCommand("cd /root/movie-app && source .env && curl -s `"https://generativelanguage.googleapis.com/v1beta/models?key=`$GEMINI_API_KEY`" > models.json && grep -o '\"name\": \"[^\"]*\"' models.json | sort -u")
Write-Host "MODELS:"
